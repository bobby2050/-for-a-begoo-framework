...
<body>
    ...
    <div class="header">
        <h1 id="header-title">{{.Data.CategoryTaoName}} </h1>
        <h2 id="header-sec-title">单选0.5，多选1分。</h2>
    ...
    <div class="next">
        <div class="next-box">
            <div class="box" lastQuestionId="1" tmpCurrentPer="1">
                下一页
            </div>
        </div>
    </div>
    ...
    <script type="application/javascript">
        ...
        $(function () {
            ...
            $.ajax({
                type: "GET",
                url: ServerUrl + "getUserQuestionLastId?userId=" + userId + "&categroyId="+getUrlParam('categroy') + "&categroy2=" + getUrlParam('category2'),
                //请求成功
                success: function (re) {
                    console.log(re)

                    $('.box').attr("lastQuestionId", re.Num)
                    ...
                    if(re.Num == 0) {
                        ...
                        $.ajax({
                            type : "GET",
                            url : ServerUrl + "getQuestionFenye?categroy="+getUrlParam('categroy')+"&userId=" + userId + "&per=1&lastQuestionId=0",
                            //请求成功
                            success : function(result) {
                                console.log(result)
                                // console.log(result.CategoryTaoName)
                                $("#header-title").html(result.CategoryTaoName)
            ...
            $("body").on('click', '#startFirstQuestion', function () {
                console.log("取消")
                var lastQuestionId = 0
                $('.box').attr('lastQuestionId', lastQuestionId) // 题的Id 1
                $.ajax({
                    type : "GET",
                    url : ServerUrl + "getQuestionFenye?categroy="+getUrlParam('categroy')+"&userId=" + userId + "&per=1&lastQuestionId=" + lastQuestionId,
                    //请求成功
                    success : function(result) {
                        // console.log(result);
                        $("#header-title").html(result.CategoryTaoName)
            ...
            $("body").on('click', '#startGoOnQuestion', function () {
                console.log("继续")
            ...
            $("body").on('click',".btnWords", function (e) {
                ...
                $.ajax({
                    type: "POST",
                    url: ServerUrl + "submitQuestion",
                    data: {
                        userId: userId,
                        questionId: questionId,
                        ok: flag,
                        categroyTaoId:getUrlParam('categroy'),
                        categroy2: getUrlParam('category2')
                    },
                    async: false,
                    method:'post',
                    //请求成功
            ...
            $(".box").click(function () {
                ...
                if (next >= Total) {
                    ...
                    $.ajax({
                        type : "GET",
                        url : ServerUrl + "getQuestionFenye?categroy="+getUrlParam('categroy')+"&userId=" + userId + "&per=" + next + "&lastQuestionId=" + lastQuestionId,
                        //请求成功
                        success : function(result) {
                            console.log(result);
